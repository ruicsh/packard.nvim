...
Helpers.describe("Lazy Loading", function()
  ...
  Helpers.it("keys = fn with require: function RHS fires via replay after stub", function()
    ...
    -- The "x" flag processes keys immediately.  With nvim_feedkeys called
    -- directly inside the expr callback (matching lazy.nvim), the replayed
    -- LHS triggers the real mapping synchronously.
    ...
    -- Verify: the side effect WAS called (replay is synchronous inside expr callback)
  ...
  Helpers.it("keys = fn: first-press in insert mode with function RHS triggers side effect", function()
    ...
    -- Verify: side effect was triggered synchronously after key replay
  ...
  Helpers.it("keys = string: first-press in insert mode moves cursor (no double-termcoding)", function()
    ...
    -- The stub fires, replaces itself with the real mapping, and replays
    -- the LHS via nvim_feedkeys inside the expr callback (no vim.schedule).
  ...
  Helpers.it("stub handles re-invocation from buffer-local fallback (blink.cmp scenario)", function()
    ...
    --
    -- The stub replays the key via nvim_feedkeys internally (not by returning
    -- the replay string from the expr mapping).  Calling the stub directly
    -- avoids the infinite re-entrancy that would happen if we went through
    -- nvim_feedkeys with the "x" flag while the buffer-local mapping is active
    -- (the stub feeds "<Ignore><c-e>", which re-triggers the buffer-local
    -- mapping that wraps the stub).
    ...
    --    The stub loads the plugin and replays the LHS via nvim_feedkeys.
    --    In headless mode, nvim_feedkeys with "i" mode places keys in the
    --    typeahead buffer without processing them, so no side effect.
    ...
    --    no-op.  nvim_feedkeys places the LHS replay in the typeahead
    --    but keys are not processed in headless mode without "x" flag.
  ...
  Helpers.it("command stub replays without E464 when plugin defines same-named command", function()
    ...
    -- The command replay (vim.schedule'd) hasn't fired yet in headless mode.
