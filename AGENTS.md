...
- **Spec function `require` resolution**: `setup_eager_load` prepends each non-cond, non-local plugin's `lua/` dir to `package.path`. This allows `keys = function() local p = require("plugin") ... end` to work during startup without sourcing the plugin's `plugin/`, `ftdetect/`, or `colors/` files. The mutation persists for the rest of the Neovim session (same model as lazy.nvim).
